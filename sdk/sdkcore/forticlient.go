...
import (
	...
	"github.com/terraform-providers/terraform-provider-fortiswitch/sdk/request"
	// "strconv"
)
...
// FortiSDKClient describes the global Fortiswitch plugin client instance
...
// NewRequest creates the request to Fortiswitch for the client
...
func (c *FortiSDKClient) CheckUP() error {
	...
	if err != nil {
		if c.Config.Auth.Hostname == "" {
			err = fmt.Errorf("Please provide the valid hostname.")
		} else if c.Config.Auth.Username == "" {
			err = fmt.Errorf("Username is wrong, please provide the correct one.")
		} else if c.Config.Auth.Password == "" {
			err = fmt.Errorf("Password is wrong, please provide the correct one.")
		}
	}
	return err
}

// UpdateDeviceVersion updates the version of FortiSwitch
// It returns version as string
func (c *FortiSDKClient) UpdateDeviceVersion() (err error) {
	client := c.Config.HTTPCon
...
// GetDeviceVersion gets the version of FortiSwitch
// It returns version as string
		...
		if result["status"] != nil {
			...
			if result["http_status"] != nil {
				...
				if result["http_status"] == 400.0 {
					err = fmt.Errorf("Bad Request - Request cannot be processed by the API (%.0f), %v", result["http_status"], result)
