...
// CreateJSONGenericAPI API operation for FortiSwitch sends request to FortiSwitch APIs.
// Returns the response from FortiSwitch .
func (c *FortiSDKClient) CreateJSONGenericAPI(params *JSONJSONGenericAPI) (res string, err error) {
	...
	if params.Json != "" {
		...
		log.Printf("FSW-fortiswitch resquest1: %s", locJSON)
	...
	log.Printf("FSW-fortiswitch response1: %s", string(body))
