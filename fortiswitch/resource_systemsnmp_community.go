...
func flattenSystemSnmpCommunityHosts6(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["interface"]; ok {
			...
			tmp["interface"] = flattenSystemSnmpCommunityHosts6Interface(i["interface"], d, pre_append, sv)
		}
...
func flattenSystemSnmpCommunityHosts(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["interface"]; ok {
			...
			tmp["interface"] = flattenSystemSnmpCommunityHostsInterface(i["interface"], d, pre_append, sv)
		}
...
func refreshObjectSystemSnmpCommunity(d *schema.ResourceData, o map[string]interface{}, sv string) error {
	...
	if err = d.Set("fswid", flattenSystemSnmpCommunityId(o["id"], d, "fswid", sv)); err != nil {
...
func flattenSystemSnmpCommunityFortiTestDebug(d *schema.ResourceData, fswdebugsn int, fswdebugbeg int, fswdebugend int) {
...
func getObjectSystemSnmpCommunity(d *schema.ResourceData, sv string) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOk("fswid"); ok {
		...
		t, err := expandSystemSnmpCommunityId(d, v, "fswid", sv)
		if err != nil {
			return &obj, err
		} else if t != nil {
			obj["id"] = t
