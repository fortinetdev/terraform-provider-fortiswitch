...
// Description: Custom field configuation.
...
func refreshObjectLogCustomField(d *schema.ResourceData, o map[string]interface{}, sv string) error {
	...
	if err = d.Set("fswid", flattenLogCustomFieldId(o["id"], d, "fswid", sv)); err != nil {
...
func flattenLogCustomFieldFortiTestDebug(d *schema.ResourceData, fswdebugsn int, fswdebugbeg int, fswdebugend int) {
...
func getObjectLogCustomField(d *schema.ResourceData, sv string) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOk("fswid"); ok {
		...
		t, err := expandLogCustomFieldId(d, v, "fswid", sv)
		if err != nil {
			return &obj, err
		} else if t != nil {
			obj["id"] = t
