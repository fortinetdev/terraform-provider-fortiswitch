...
func refreshObjectSystemTosBasedPriority(d *schema.ResourceData, o map[string]interface{}, sv string) error {
	...
	if err = d.Set("fswid", flattenSystemTosBasedPriorityId(o["id"], d, "fswid", sv)); err != nil {
...
func flattenSystemTosBasedPriorityFortiTestDebug(d *schema.ResourceData, fswdebugsn int, fswdebugbeg int, fswdebugend int) {
...
func getObjectSystemTosBasedPriority(d *schema.ResourceData, sv string) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOk("fswid"); ok {
		...
		t, err := expandSystemTosBasedPriorityId(d, v, "fswid", sv)
		if err != nil {
			return &obj, err
		} else if t != nil {
			obj["id"] = t
